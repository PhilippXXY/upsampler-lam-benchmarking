...
class SELDMetrics:
    def __init__(self, doa_threshold=20, nb_classes=11, average="macro"):
        """
        This class implements both the class-sensitive localization and
        location-sensitive detection metrics.
        Additionally, based on the user input,
        the corresponding averaging is performed within the segment.
    ...
    def compute_seld_scores(self):
        ...
        :return: returns both location-sensitive detection scores and
        class-sensitive localization scores
        """
        ...
        if self._average == "micro":
            ...
            F = self._TP.sum() / (
                eps + self._TP.sum() + self._FP_spatial.sum()
        ...
        return ER, F, LE, LR, SELD_scr, classwise_results # type: ignore

    def update_seld_scores(self, pred, gt):
        """
        Implements the spatial error averaging according to equation 5 in the paper [1]
        (see papers in the title of the code).
        Adds the multitrack extensions proposed in paper [2]

        The input pred/gt can either both be Cartesian or Degrees

        :param pred: dictionary containing class-wise prediction results
        for each N-seconds segment block
            ...
            for class_cnt in range(self._nb_classes):
                ...
                if class_cnt in gt[block_cnt] and class_cnt in pred[block_cnt]:
                    ...
                    if len(matched_track_dist) == 0:
                        ...
                        # This occurs when the predicted DOAs are not aligned
                        # frame-wise to the reference DOAs
                        loc_FN += nb_pred_doas # type: ignore
                        ...
                        # in the multi-instance of same class scenario, if the number of predicted
                        # tracks are greater
                        # than reference tracks count as FP, if it less than reference count as FN
                        if nb_pred_doas > nb_gt_doas: # type: ignore
                            # False positive
                            loc_FP += (nb_pred_doas - nb_gt_doas) # type: ignore
                            self._FP[class_cnt] += (nb_pred_doas - nb_gt_doas) # type: ignore
                            self._DE_FP[class_cnt] += (nb_pred_doas - nb_gt_doas) # type: ignore
                        elif nb_pred_doas < nb_gt_doas: # type: ignore
                            # False negative
                            loc_FN += (nb_gt_doas - nb_pred_doas) # type: ignore
                            self._FN[class_cnt] += (nb_gt_doas - nb_pred_doas) # type: ignore
                            self._DE_FN[class_cnt] += (nb_gt_doas - nb_pred_doas) # type: ignore
                elif class_cnt in gt[block_cnt] and class_cnt not in pred[block_cnt]:
                    # False negative
                    loc_FN += nb_gt_doas # type: ignore
                    ...
                    # False positive
                    loc_FP += nb_pred_doas # type: ignore
...
def least_distance_between_gt_pred(gt_list, pred_list):
    ...
    coordinate pairs resulting in a matrix of distances, where one axis represents the number
    of groundtruth coordinates and the other the predicted coordinates.
    The number of estimated peaks need not be the same as in groundtruth,
    thus the distance matrix is not always a square matrix.
    We use the hungarian algorithm to find the least cost in this distance matrix.
    ...
    if gt_len and pred_len:
        if len(gt_list[0]) == 3:  # Cartesian
            ...
            cost_mat[ind_pairs[:, 0], ind_pairs[:, 1]] = distance_between_cartesian_coordinates(
                x1, y1, z1, x2, y2, z2
            )
        else:
            az1, ele1, az2, ele2 = (
            ...
            cost_mat[ind_pairs[:, 0], ind_pairs[:, 1]] = distance_between_spherical_coordinates_rad(
                az1, ele1, az2, ele2
            )
